...
#train
random_forest = RandomForestClassifier(n_estimators=200, bootstrap=True,max_features=None,min_samples_leaf= 50, random_state=0)
random_forest.fit(df_fin_train , y_train.values.ravel())

y_predict = random_forest.predict(df_fin_test)
y_pred_df = pd.DataFrame(y_predict)
y_pred_df.index.name = 'Id'
y_pred_df.columns = ['Label']
y_pred_df.to_csv('random_forest_ult_pred.csv')
